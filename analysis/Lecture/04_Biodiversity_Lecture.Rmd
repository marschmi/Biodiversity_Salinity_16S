...
knitr::opts_chunk$set(echo = TRUE, 
                      fig.path = "../../figures/Lecture/04_Biodiversity_Lecture/",
                      fig.align = "center")
```
...
#save(iNEXT_data, file = "data/Lecture/04_Biodiversity_Lecture/iNEXT_data.RData")
```

Since iNEXT takes a while to run, we will load in the data that was caluculated before! 

```{r load-iNEXT-data}
load("data/Lecture/04_Biodiversity_Lecture/iNEXT_data.RData")
...
ggiNEXT(iNEXT_data, type = 1, facet.var = "Order.q") + 
  ...
  scale_shape_manual(values = base::rep(17, nsamples(preprocessed_physeq))) + 
  theme_bw() + 
...
iNEXT_manual_df %>%
  ...
  ggplot(aes(x = m, y = qD, color = salinity_group, group = names)) + 
  geom_line() + 
  # Facet by station 
  facet_grid(Order.q~salinity_group, scales = "free") + 
...
- *Richness: q = 0*  
    ...
    - There does not appear to be much difference between the salinity groups in the total richness.   
    - However, there does appear to be a lower outlier sample in the Mid group! Again, we will keep an eye on this sample.
- *Shannon Diversity: q = 1*
    - Here, the trends that were starting to be witnessed in the Shannon diversity are more amplified.
- *Simpson's Diversity: q = 2* 
    - The **Very Low** seems to generally be the highest with 40-60 dominant species versus.  
    - The **Low** and **Mid** groups appears to have the lowest Simpson's diversity as the plateau is ~10-30 dominant taxa.  
    - There are two samples in the **High** salinity group that have much larger Simpson Diversity values compared to other samples. Wonder which sample those are! 
...
distinct_metadata_df <- 
  metadata_df %>%
  # There are some redundant samples that are specific to the in class dataset, let's remove them
...
dunn_station_salinity <- 
  obs_whole_rich_df %>%
  dunn_test(salinity_psu ~ salinity_group, p.adjust.method = "fdr") %>%
  add_xy_position(x = "salinity_group")
...
# Salinity by station 
salinity_ggboxplot <- 
  ggboxplot(obs_whole_rich_df, x = "salinity_group", y = "salinity_psu", 
          color = "salinity_group", fill = "salinity_group", alpha = 0.3,
          outlier.shape = NA) + 
  # Add points
  geom_jitter(aes(color = salinity_group)) + 
  scale_color_manual(values = salinity_colors) + 
...
obs_div_df %>%
  ggplot(aes(x = salinity_group, y = qD, fill = salinity_group, 
...
anova_salinityGroup_rich <- 
  aov(qD ~ salinity_group, data = obs_rich_df)
...
# Richness by station 
rich_ggboxplot <- 
  ggboxplot(obs_rich_df, x = "salinity_group", y = "qD", 
          color = "salinity_group", fill = "salinity_group", alpha = 0.3,
          outlier.shape = NA) + 
  # Add points
  geom_jitter(aes(color = salinity_group)) + 
  ...
  geom_pwc(
    aes(group = salinity_group), tip.length = 0, hide.ns = TRUE,
...
anova_salinityGroup_simps <- 
  aov(qD ~ salinity_group, data = obs_simps_df)
...
# Simpson by station 
simps_ggboxplot <- 
  ggboxplot(obs_simps_df, x = "salinity_group", y = "qD", 
          color = "salinity_group", fill = "salinity_group", alpha = 0.3,
          outlier.shape = NA) + 
  # Add points
  geom_jitter(aes(color = salinity_group)) + 
...
On the left, we can see that ***only*** the richness of the lowest and highest salinity groups are significantly different. However, on the right when evaluating Simpson's Index (dominant ASVs), we can see that there is quite a bit of a differences between salinity groups and that there is more of a "U-shaped" differences than a general trend down on the left.
...
1. This s where you can clearly see some variables are correlated! For example, `salinity_psu` and `SpCond_mS` are perfectly correlated. Therefore, we should be careful to compare these two!  
2. `percent_DO` and `DO_mgL` are two measurements of dissolved oxygen arand are also correlated.  
3. Finally, `turbidity_NTU` and `chlorophyll_ugL` also appear correlated and we should be mindful of these!
...
obs_div_df %>%
  ...
  geom_point(aes(color = salinity_group)) + 
  scale_color_manual(values = salinity_colors) + 
