...
```{r load-data}
load("data/Lecture/05_Ordination_Lecture/scaled_physeq.RData")
...
```{r plot-Top10-Phyla-salGroup, fig.width=12, fig.height=6}
...
actino_phylum_salGroup + actino_phylum_salinity + 
...
B. Abundance shows a strong negative relationship with salinity, consistent with a freshwater preference. This taxon appears to be sensitive to increasing salinity, with little presence in high-salinity samples.
...
# Plot by salinity_group 
cyano_phylum_salGroup <- 
  phylum_df %>%
...
cyano_phylum_salinity <- 
  ...
  # build the plot 
  ggplot(aes(x = salinity_psu, y = Abundance)) + 
  geom_point(aes(color = salinity_group, shape = date)) + 
  theme_bw() + 
...
cyano_phylum_salGroup + cyano_phylum_salinity + 
  plot_layout(guides = "collect") +
  plot_annotation(tag_levels = "A")
```
...
A. Cyanobacteriota relative abundance varies across estuarine stations, with generally higher median abundances observed in mid-salinity environments compared to the more freshwater (Copano) and marine-influenced (Shipping Channel) sites.
...
verruco_phylum_salGroup + verruco_phylum_salinity + 
  plot_layout(guides = "collect") +
  plot_annotation(tag_levels = "A")
```
...
# Narrow in on a specific group
# Bacteroidota - y: abundance, x: salinity_group, dot plot + boxplot

# Plot by salinity_group 
bacteroidota_phylum_salGroup <- 
  phylum_df %>%
...
bacteroidota_phylum_salGroup + bacteroidota_phylum_salinity + 
  plot_layout(guides = "collect") +
  plot_annotation(tag_levels = "A")
```
...
## C1. Actinomycetota Genera 

```{r actino-genus, fig.width=6, fig.height=6}
# Actinobacteriota
# Plot genus 
actino_genus_salGroup <- 
  genus_df %>%
  dplyr::filter(Phylum == "Actinomycetota") %>%
...
actino_genus_salGroup / actino_genus_salinity + 
  plot_layout(guides = "collect") +
  plot_annotation(tag_levels = "A")
```
...
This figure shows the relative abundance of two dominant Actinomycetota genera—CL500-29 marine group and hgcI clade—across salinity groups (A) and a salinity gradient (B).
...
cyano_genus_salinity <- 
  ...
  ggplot(aes(x = salinity_psu, y = Abundance)) + 
  facet_wrap(.~Genus, scales = "free_y", nrow = 1) + 
  geom_point(aes(color = salinity_group)) +  theme_bw() + 
  geom_smooth(method = "lm",  formula = y ~ poly(x, 2)) + 
  labs(title = "Cyanobacteriota Genus Abundance") + 
  scale_color_manual(values = salinity_colors) + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1, vjust = 1),
...
# Collect the Cyanobacteriota Plots
cyano_genus_salGroup / cyano_genus_salinity + 
  plot_layout(guides = "collect") +
  plot_annotation(tag_levels = "A")
```
...
## D1. Actinomycetota ASVs 

```{r actino-asvs, fig.width=10, fig.height=8}
...
# Collect the Actinomycetota Plots
actino_asv_salGroup / actino_asv_salinity + 
  plot_layout(guides = "collect") +
  plot_annotation(tag_levels = "A")
```
...
A. **Relationship to Salinity Group:**  
...
# Collect the Actinomycetota Plots
cyano_asv_salGroup / cyano_asv_salinity + 
  plot_layout(guides = "collect") +
  plot_annotation(tag_levels = "A")
```
...
A. **Relationship to Salinity Group:** This spatial pattern reinforces the environmental filtering hypothesis, where salinity selects for different cyanobacterial taxa. In the samples with the highest salinity, a spike in Synechococcus ASVs and a concomitant decline or absence of many Cyanobium ASVs. However, samples with lower salinities are dominated by Cyanobium ASVs.  
...
## D3. Verrucomicrobiota 

```{r verruco-asvs, fig.width=6, fig.height=6}
# Calculate top couple of ASVs 
# Make a list of phyla the top phyla 
top_verruco_ASVs <- 
  ASV_df %>%
...
# Collect the Actinomycetota Plots
verruco_asv_salGroup / verruco_asv_salinity + 
  plot_layout(guides = "collect") +
  plot_annotation(tag_levels = "A")
```
...
```{r bacter-asvs, fig.width=6, fig.height=10}
# Calculate top couple of ASVs 
# Make a list of phyla the top phyla 
top_bacter_ASVs <- 
  ASV_df %>%
...
# Collect the Actinomycetota Plots
bacter_asv_salGroup / bacter_asv_salinity + 
  plot_layout(guides = "collect") +
  plot_annotation(tag_levels = "A")
```
