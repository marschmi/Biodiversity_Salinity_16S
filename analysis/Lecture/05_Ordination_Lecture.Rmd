...
load("data/Lecture/03_PreProcessing_Lecture/preprocessed_phyloseq.RData")
...
<span style="color: red;">INTERPRETATION #2:  Evaluate the raw read count plots above. What did you scale the read counts to in your dataset? Why did you choose that number? Do you think it is sufficient to perform a beta-diversity analysis and accurately compare your data to one another?</span>
...
sorensen_pcoa_plot <- 
  ...
  geom_point(size = 5, alpha = 0.5, aes(color = salinity_group)) + 
  theme_bw() + 
...
<span style="color: red;">INTERPRETATION #6: Plot your data using NMDS. Interpret what you can conclude from the NMDS.</span>
...
<span style="color: red;">INTERPRETATION #7: Plot both the PCoA and NMDS together. Which of the two visualizations work best for your data? Explain your reasoning.</span>
...
sorensen_salGroup_adonis_terms1 <- adonis2(scaled_sorensen_dist ~ salinity_group * date , data = metadata_df, by = "terms")
sorensen_salGroup_adonis_terms1

# Check the order of the terms
sorensen_salGroup_adonis_terms2 <- adonis2(scaled_sorensen_dist ~ date * salinity_group, data = metadata_df, by = "terms")
...
sorensen_salGroup_adonis_margin <- adonis2(scaled_sorensen_dist ~ date * salinity_group, data = metadata_df, by = "margin")
sorensen_salGroup_adonis_margin
```
...
4. Finally, the total residiuals of the model is ~`r round(sorensen_salGroup_adonis_terms1$R2[4]*100, digits = 0)`% of the variation remains unexplained by these variables, which is typical in complex environmental microbiome datasets.
...
sorensen_salinity_adonis_terms1 <- adonis2(scaled_sorensen_dist ~ date * salinity_psu, data = metadata_df, by = "terms")
sorensen_salinity_adonis_terms1

# Check different order 
sorensen_salinity_adonis_terms2 <- adonis2(scaled_sorensen_dist ~ salinity_psu * date, data = metadata_df, by = "terms")
...
# Bray-Curtis
## ALWAYS check and confirm the order of your terms and how they interact with each other.  
bray_salGroup_adonis_terms1 <- adonis2(scaled_bray_dist ~ salinity_group * date, data = metadata_df, by = "terms")
...
bray_salGroup_adonis_margin <- adonis2(scaled_bray_dist ~ date * salinity_group, data = metadata_df, by = "margin")
bray_salGroup_adonis_margin
```
...
1. **Station** is the strongest factor, explaining ~`r round(bray_salGroup_adonis_terms1$R2[1]*100, digits = 0)`% of the variation in microbial community composition, which is statistically significant based on the p-value of <`r bray_salGroup_adonis_terms1$"Pr(>F)"[1]`. This suggests spatial differences among stations play a major role in structuring the community.
2. **Date** also contributes significantly (~`r round(bray_salGroup_adonis_terms1$R2[2]*100, digits = 0)`% of variation, with a p-value of <`r bray_salGroup_adonis_terms1$"Pr(>F)"[2]`), indicating meaningful temporal shifts in community composition across the sampling window.
...
4. Finally, the total residiuals of the model is ~`r round(bray_salGroup_adonis_terms1$R2[4]*100, digits = 0)`% of the variation remains unexplained by these variables, which is typical in complex environmental microbiome datasets.
...
- The higher R^2^ for station in Bray-Curtis (~`r round(bray_salGroup_adonis_terms1$R2[1]*100, digits = 0)`% vs. ~`r round(sorensen_salGroup_adonis_terms1$R2[1]*100, digits = 0)`%) indicates that abundance shifts across space are even more pronounced than presence/absence alone would suggest.
...
<span style="color: red;">INTERPRETATION #8: Using the above example to guide you, run the appropriate PERMANOVA for your scientific question and interpret the results. What biological conclusion can you draw from your PERMANOVA results?</span>
